...
frame :: proc "c" (dt: f32) {
    ...
    surfacte_texture := wgpu.SurfaceGetCurrentTexture(state.surface)

    switch surfacte_texture.status {
    case .SuccessOptimal, .SuccessSuboptimal:
        // all good, could handle suboptimal here.
    case .Timeout, .Outdated, .Lost:
        // skip this frame, and re-configure surface.
        if surfacte_texture.texture != nil {
            wgpu.TextureRelease(surfacte_texture.texture)
        }
        resize()
        return
    case .OutOfMemory, .DeviceLost, .Error:
        // fatal error
        fmt.panicf("[triangle] get_current_texture status=%v", surfacte_texture.status)
    }
    defer wgpu.TextureRelease(surfacte_texture.texture)

    frame := wgpu.TextureCreateView(surfacte_texture.texture, nil)
